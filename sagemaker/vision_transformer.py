...
class PatchEmbeddingWithClassToken(tf.keras.layers.Layer):
    def __init__(self, embedding_dim, lr_weight_decay, **kwargs):
        super().__init__(**kwargs)
        self.embedding_layer = tf.keras.layers.Dense( # a simple dense layer for projection
            units=embedding_dim, 
            activation="gelu", 
            kernel_regularizer=tf.keras.regularizers.l2(lr_weight_decay)
        )
...
class EncoderLayer(tf.keras.layers.Layer):
    def __init__(self, num_heads, embedding_dim, mlp_dim, lr_weight_decay, dropout_rate=0.1, **kwargs):
        super().__init__(**kwargs)
        ...
        self.mlp = tf.keras.Sequential([
            tf.keras.layers.Dense( # paper user GELU activation
                units=mlp_dim, 
                activation="gelu",
                kernel_regularizer=tf.keras.regularizers.l2(lr_weight_decay)
            ),
...
class ClassificationHead(tf.keras.layers.Layer):
    def __init__(self, num_classes, mlp_dim, dropout_rate, lr_weight_decay, **kwargs):
        super().__init__(**kwargs)
...
class ViT(tf.keras.Model):
    def __init__(
        ...
        **kwargs
        ...
        self.patch_embedding_class_token = PatchEmbeddingWithClassToken(embedding_dim, lr_weight_decay)
        self.positional_embedding = PositionalEmbedding(
            num_patches=(input_shape[0] // patch_size) ** 2 + 1, # +1 includes [class] embedding
            embedding_dim=embedding_dim
        )
        self.encoder_stack = [
            EncoderLayer(num_heads, embedding_dim, mlp_dim, lr_weight_decay, dropout_rate)
            for _ in range(num_layers)
        ]
        self.class_token_norm = tf.keras.layers.LayerNormalization()
        self.classification_head = ClassificationHead(num_classes, clf_mlp_dim, dropout_rate, lr_weight_decay)
